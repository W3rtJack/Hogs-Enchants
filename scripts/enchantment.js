...
export class Enchantment {
    constructor (id,name,maxLevel,slot,cancelList=[]){
        this.id = id;
        this.name = name;
    ...
    removeItem(item){
        const l = item.getDynamicProperty(`enchant.${this.id}.level`) > 0 ? item.getDynamicProperty(`enchant.${this.id}.level`) : 0
        item.setDynamicProperty(`enchant.${this.id}.valid`,false)
        item.setDynamicProperty(`enchant.${this.id}.level`,0)
        return l
    }
...
// Cancel list is an array of the enchantments it cannot be paired with. Being in the form of enchant ids
// For slot use the dict slots below
export function registerEnchant(id,name,maxLevel,slot,cancelList=[]){
    var s = ""
        ...
        if (player != undefined){
            if (world.getDynamicProperty("ench.start") != true){
                ...
                regName.forEach(name=> {
                    ...
                    loc.y = 100;
                    const ench = world.getPlayers()[0].dimension.spawnEntity("hog:enchant_registry",loc);
                    ench.nameTag = name;
                })
            }
            ...
            // If your for some reason copying from this source uncomment this next section
